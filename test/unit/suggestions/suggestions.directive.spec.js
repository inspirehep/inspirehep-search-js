...
describe('Check suggestions directive', function() {
  ...
  beforeEach(
    inject(function(_$compile_, _$rootScope_) {
      ...
      // Populate invenioSearchResults
      scope.vm = {}
      scope.vm.invenioSearchResults = { 'warnings': [{"query_suggestion": "MalformedQuery"}]}
      scope.$digest();
    }));

  it('Test if the template is loaded', function() {
    // Check that the compiled element contains the templated content
    expect(template.html()).to.contain('class="suggestion_icon"');
  });
