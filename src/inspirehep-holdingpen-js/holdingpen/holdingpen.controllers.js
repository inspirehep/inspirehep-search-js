...
(function (angular) {
  function HoldingPenSelectionCtrl($scope, Hotkeys, HoldingPenRecordService, $window) {
    ...
    function toggleAll() {
      ...
      if (remove_all) {
        ...
        angular.forEach($scope.$parent.vm.invenioSearchResults.hits.hits, function (record) {
          if (!isChecked(record._id)) {
            _add_record_method(record);
            $scope.vm.selected_records.push(+record._id);
            if (record._source._workflow.data_type) {
              if (!(record._source._workflow.data_type in $scope.vm.selected_record_decisions)) {
  ...
  HoldingPenSelectionCtrl.$inject = [
    '$scope',
    'Hotkeys',
